            ...
            if (sender instanceof Player) {
                ...
                if (args.length == 1) {
                    if (args[0].equalsIgnoreCase("@m")) {
                        ...
                        player.sendMessage(ChatColor.GREEN + "All custom mobs have been removed and lastId reset to 0.");
                    } else {
                        try {
                            int id = Integer.parseInt(args[0]);
                            customMobManager.removeCustomMob(id);
                            player.sendMessage(ChatColor.GREEN + "Custom mob with ID " + id + " has been removed.");
                        } catch (NumberFormatException e) {
                            String nameTag = args[0];
                            customMobManager.removeCustomMobByNameTag(nameTag);
                            player.sendMessage(ChatColor.GREEN + "Custom mob(s) with name tag " + nameTag + " have been removed.");
                        }
                    }
                } else {
                    player.sendMessage(ChatColor.RED + "Usage: /removecustommob [nametag|@m]");
                }
                return true;
            }
        }
